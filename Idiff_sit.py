...
# Idiff_sit
def Idiff_motion_eFC_all(GROUP_DATA_FOLDER,sit_cmp,freq,subc,corr_type,eFC_mean_sort):
    k=0
        ...
        for filename in filenames:
            ...
            if path_mat[-4:] == '.mat':
                ...
                VeFC_sitd,trn_sitd = VeFC_mst_motion_dyad(data_mat,sit_cmp,freq,
                                                 subc,corr_type)
                I_diff,I_self,I_other,I_self_mean,per_change,per_diff=Idiff.get_Idiff(VeFC_sitd,trn_sitd,'pearsonr')
                # 保留两位小数并记录数据
                I_concat = [I_diff,I_self_mean,I_other,per_change,per_diff]
                I_eFC_dyad = np.around(I_concat,2)                
            # 拼接各个dyad的Idiff
            if k==0:
...
def Idiff_motion_nFC_all(GROUP_DATA_FOLDER,motion_comp,freq,subc,corr_type):
    k=0
    for filepath,dirnames,filenames in os.walk(GROUP_DATA_FOLDER):
        for filename in filenames:
            path_mat = os.path.join(filepath,filename)
            ...
            if path_mat[-4:] == '.mat':
                ...
                VnFC_sitd,trn_sitd = VnFC_motion_dyad(data_mat,motion_comp,freq,
                                                 subc)
                I_diff,I_self,I_other,I_self_mean,per_change,per_diff=Idiff.get_Idiff(VnFC_sitd,trn_sitd,'pearsonr')
                # 保留两位小数并记录数据
                I_concat = [I_diff,I_self_mean,I_other,per_change,per_diff]
                I_nFC_dyad = np.around(I_concat,2)
            # 拼接各个dyad的Idiff
            if k==0:
                I_nFC_all=I_nFC_dyad
                k=1
            else:
                I_nFC_all=np.c_[I_nFC_all,I_nFC_dyad]
